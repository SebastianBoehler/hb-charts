...
export function formatTimeAxisLabel(timestamp: number, visibleSpan: number) {
  ...
  if (visibleSpan <= 120 * 24 * 60 * 60 * 1_000) return `${day}\u00a0${month}`;
...
export function buildTimeAxisTicks({
  ...
  if (!bars.length || count <= 0 || dataWidth <= 0) return [];
  ...
  const indices = Array.from({ length: segments + 1 }, (_, position) =>
    Math.round(first + ((last - first) * position) / segments),
