...
export type StandardIndicatorId =
  | "atr"
  | "bollinger"
  | "ema"
  | "macd"
...
export function buildStandardIndicators(
  bars: readonly ChartBar[],
  active: readonly StandardIndicatorId[],
): StandardIndicatorResult {
  ...
  if (ids.has("ema")) {
    layers.push(
      { id: "ema-12", kind: "line", label: "EMA 12", values: ema(closes, 12) },
      { id: "ema-48", kind: "line", label: "EMA 48", values: ema(closes, 48) },
    );
  }
  if (ids.has("sma")) {
  ...
  if (ids.has("bollinger")) {
    ...
    layers.push(
      { id: "bollinger-upper", kind: "line", label: "Bollinger upper", values: bands.upper },
      { id: "bollinger-middle", kind: "line", label: "Bollinger middle", values: bands.middle },
      { id: "bollinger-lower", kind: "line", label: "Bollinger lower", values: bands.lower },
    );
  }
  if (ids.has("volume")) {
  ...
  if (ids.has("macd")) {
    ...
    layers.push(
      { id: "macd-line", kind: "line", paneId: "macd", values: values.line },
      { id: "macd-signal", kind: "line", paneId: "macd", values: values.signal },
      { colorMode: "value-sign", id: "macd-histogram", kind: "histogram", paneId: "macd", values: values.histogram },
    );
  }
  return { layers, panes };
}
