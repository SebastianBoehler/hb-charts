...
export const DEFAULT_PRICE_SCALE: PriceScaleMode = "linear";

export function parsePriceScale(value: unknown): PriceScaleMode {
  return value === "log" ? "log" : DEFAULT_PRICE_SCALE;
}

export function isPriceScaleAvailable(
...
function logValue(value: number) {
  if (!Number.isFinite(value) || value <= 0) {
    throw new RangeError(
      "Log price scale requires finite, strictly positive values.",
    );
  }
  return Math.log(value);
}

function transformedRange(range: ChartPriceRange, mode: PriceScaleMode) {
  if (mode === "linear") return range;
...
function rawRange(min: number, max: number, mode: PriceScaleMode) {
  if (mode === "linear") return { max, min };
...
export function normalizePriceRange(
  ...
  max: number,
  paddingRatio = 0.04,
  mode: PriceScaleMode = DEFAULT_PRICE_SCALE,
): ChartPriceRange {
  if (mode === "log") {
...
export function priceRangeForValues(
  values: number[],
  mode: PriceScaleMode = DEFAULT_PRICE_SCALE,
) {
...
export function scalePriceRange(
  ...
  const nextSpan = Math.max(
    ...
    Math.min(
      maximumSpan,
      currentSpan * Math.max(MIN_SCALE_FACTOR, Math.min(MAX_SCALE_FACTOR, factor)),
    ),
  );
  const anchorRatio = Math.max(
    0,
    Math.min(1, (projectedAnchor - transformed.min) / currentSpan),
  );
  const min = projectedAnchor - anchorRatio * nextSpan;
  return rawRange(min, min + nextSpan, mode);
}

export function shiftPriceRange(
  range: ChartPriceRange,
  deltaRatio: number,
) {
