...
function slicedLayer(layer, start, end) {
...
export function drawChart(canvas, options) {
    ...
    if (!bars.length || bounds.width <= 0 || bounds.height <= 0)
        return null;
    ...
    ].filter((value) => value !== null && Number.isFinite(value) && (mode === "linear" || value > 0));
    ...
    const x = (index) => CHART_PADDING.left + (index / slots) * width;
    const y = (value) => yForPrice(range, value, CHART_PADDING.top, priceHeight, mode);
    const projection = {
    ...
    priceLayers.forEach((layer) => {
        if (layer.kind === "line")
            drawLine(ctx, layer, x, y, palette);
    });
...
//# sourceMappingURL=render-chart.js.map