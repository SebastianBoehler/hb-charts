export function snapStrokeCoordinate(value, devicePixelRatio, cssLineWidth = 1) {
...
export function snapFillCoordinate(value, devicePixelRatio) {
    return Math.round(value * devicePixelRatio) / devicePixelRatio;
}
export function containingBarIndex(bars, time) {
    if (!bars.length || !Number.isFinite(time) || time < bars[0].time)
        return -1;
    let low = 0;
    ...
    if (containing < bars.length - 1)
        return containing;
    if (time === bars[containing].time)
        return containing;
    if (bars.length < 2)
        return -1;
