export const CHART_PADDING = { left: 12, right: 62, top: 18 };
export const TIME_AXIS_HEIGHT = 28;
export function buildPaneLayout(height, panes, collapsed = new Set()) {
    const available = Math.max(0, height - CHART_PADDING.top - 12 - 2 - TIME_AXIS_HEIGHT);
    if (!panes.length)
        return { paneHeight: 0, panes: [], priceHeight: available };
