...
export function ema(values, length) {
    if (!Number.isInteger(length) || length < 1)
        throw new RangeError("EMA length must be positive.");
...
export function sma(values, length) {
    if (!Number.isInteger(length) || length < 1)
        throw new RangeError("SMA length must be positive.");
    const output = empty(values.length);
    let total = 0;
    values.forEach((value, index) => {
        total += value;
...
export function atr(bars, length) {
    if (!Number.isInteger(length) || length < 1)
        throw new RangeError("ATR length must be positive.");
...
export function rsi(values, length) {
    if (!Number.isInteger(length) || length < 1)
        throw new RangeError("RSI length must be positive.");
    const output = empty(values.length);
...
export function stochastic(bars, length) {
...
export function bollinger(values, length = 20, multiplier = 2) {
    ...
    values.forEach((_, index) => {
        ...
        const deviation = Math.sqrt(window.reduce((total, value) => total + (value - mean) ** 2, 0) / length);
...
//# sourceMappingURL=indicator-math.js.map