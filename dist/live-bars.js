...
export function barBucketStart(time, resolution) {
    if (resolution === "1D")
        return utcDayStart(time);
    if (resolution === "1W") {
...
export function mergeRealtimeBar(bars, realtimeBar, resolution, maxBars = 5_000) {
    const nextBar = { ...realtimeBar, time: barBucketStart(realtimeBar.time, resolution) };
    if (!isValidBar(nextBar))
        return bars;
    if (!bars.length)
        return [nextBar];
    const last = bars.at(-1);
    if (nextBar.time > last.time)
        return [...bars, nextBar].slice(-maxBars);
    if (nextBar.time === last.time)
...
export function mergeHistoryBars(historicalBars, currentBars, maxBars = 5_000) {
...
//# sourceMappingURL=live-bars.js.map